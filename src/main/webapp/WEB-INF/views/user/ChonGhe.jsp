...
<body>
	<div class="container">
		<header class="navbar navbar-expand-lg navbar-light col"
			...
			<button class="btn btn-info"
				style="width: 150px; height: 50px; border-radius: 15px; background: lightskyblue; color: aliceblue">Đăng
				Xuất</button>
		</header>
		<div class="col-12">
			<div class="center">
				<div class="tickets">
					...
					<div class="price">
						...
						<div>
							<button type="button" class="btn btn-success bookTicketBtn"
								onclick="bookTicket()" disabled>Đặt vé</button>
						</div>
					</div>
		...
		<!-- Form để đặt vé cho một hoặc nhiều ghế -->
		<form id="bookTicketFormGhe" method="POST"
			action="user/DatVe.htm">
			<input type="hidden" name="maGhes" id="maGhesInput" value="">
    ...
    function bookTicket() {
        ...
        document.getElementById('maGhesInput').value = selectedSeats.join(',');
        document.getElementById('bookTicketFormGhe').submit();
    }

    let seats = document.querySelector(".all-seats");
    let tickets = seats.querySelectorAll("input");
    let countElement = document.querySelector(".count");
    let bookBtn = document.querySelector(".bookTicketBtn");

    tickets.forEach((ticket) => {
    	ticket.addEventListener("mouseenter", () => {
    	    const priceLabel = ticket.parentElement.querySelector('.price-label');
    	    const seatPrice = ticket.parentElement.querySelector('.seat-price');
    	    const seatIndex = Array.from(ticket.parentElement.querySelectorAll('input[type="checkbox"]')).indexOf(ticket);

    	    if (seatIndex >= 0 && seatIndex < phong.giaGhes.length) {
    	    	const loaiGhe = listLoaiGhe[seatIndex];
    	    	const giaGhe = loaiGhe.giaGhe;
    	        const tenLoaiGhe = ghe.loaiGhe.tenLoaiGhe;
    	        priceLabel.style.display = "inline";
    	        seatPrice.textContent = `${tenLoaiGhe}: ${giaGhe} VNĐ`;
    	    }
    	});
        ...
        ticket.addEventListener("change", () => {
            ...
            // Enable or disable book button based on the count of selected seats
